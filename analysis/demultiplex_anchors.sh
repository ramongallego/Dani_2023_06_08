...
################################################################################
  ...
  awk 'NR % 4 == 2 {print f,length, FILENAME } {f=$1}' "${READ1}"  >> "$OUTPUT_DIR"/seq.lengths.txt 
   
   wc -l "$OUTPUT_DIR"/seq.lengths.txt

  echo "Number of sequences after size selection and anchoring"

  awk 'NR % 4 == 2 {print f,length, FILENAME } {f=$1}' "${READ1}".anchored.fastq >> "$OUTPUT_DIR"/seq.lengthsb.txt
  wc -l "$OUTPUT_DIR"/seq.lengthsb.txt
  cat "$OUTPUT_DIR"/seq.lengthsb.txt >> "$OUTPUT_DIR"/seq.lengths.txt

	echo "Number of sequences reversed"

	grep rc$ "${READ1}".anchored.fastq | wc -l

  echo "A few more sequences were anchored after lowering the error tolerance to 0.35"

  awk 'NR % 4 == 2 {print f,length, FILENAME } {f=$1}' "${READ1}".anchored_second_round.fastq > "$OUTPUT_DIR"/seq.lengthsb.txt
   wc -l "$OUTPUT_DIR"/seq.lengthsb.txt
   cat "$OUTPUT_DIR"/seq.lengthsb.txt >> "$OUTPUT_DIR"/seq.lengths.txt
   rm "$OUTPUT_DIR"/seq.lengthsb.txt
  cat "${READ1}".anchored_second_round.fastq >> "${READ1}".anchored.fastq
  rm "${READ1}".anchored_second_round.fastq



#     cutadapt -g "file:"${BARCODES_DIR}"/barcodes_P5_anchored.fasta;min_overlap=6" -o "${DEMULT_DIR}"/"${FILE1[i]}"/{name}_round1.fastq \
#    --quiet --untrimmed-output "${OUTPUT_DIR}"/${BASE1}_nop5.fastq -e 0.1 "${READ1}".anchored.fastq --cores="${N_CORES}"

	cutadapt -g "file:"${BARCODES_DIR}"/barcodes_P5.fasta;min_overlap=8" -o "${DEMULT_DIR}"/"${FILE1[i]}"/{name}_round1.fastq \
	 ...
	 for file in "${n_files[@]}"; do
		 ...
		 awk -v VARIABLE=$file 'NR%4==2 {print f,length, VARIABLE } {f=$1}' $file >> "$OUTPUT_DIR"/seq.lengths.txt
...
## TODO: CAAGCAGAAGACGGCATACGAGAT can works as a way of anchoring the p7
	...
	cutadapt -g "CAAGCAGAAGACGGCATACGAGAT;min_overlap=10" \
         -o  "${file}".anchored.rc.fastq \
         "${file}".rc.fastq  --discard-untrimmed  -e 0.3 --cores="${N_CORES}" >> "${LOGFILE}"
				 # Change adaptor anchoring -e value

 awk -v VARIABLE=$file.anchored.rc.fastq 'NR%4==2 {print f,length, VARIABLE } {f=$1}' $file.anchored.rc.fastq >> "$OUTPUT_DIR"/seq.lengths.txt

	cutadapt -g "file:"${BARCODES_DIR}"/barcodes.p7.for.rc.fasta;min_overlap=8" \
				 -o "${DEMULT_DIR}"/"${FILE1[i]}"/"${BASE_OUTPUT}"_{name}_round2_rc.fastq --no-indels \
				 --quiet --untrimmed-output "${file}"_nop7.fastq -e 0 "${file}".anchored.rc.fastq --cores="${N_CORES}" >> "${LOGFILE}"
...
n_files2=("${DEMULT_DIR}"/"${FILE1[i]}"/"${BASE_OUTPUT}"_*_round2_rc.fastq)
	...
	awk -v VARIABLE=$file2 'NR%4==2 {print f,length, VARIABLE } {f=$1}' $file2 >> "$OUTPUT_DIR"/seq.lengths.txt
	#	echo "${file2}"
